...
knitr::include_graphics("figs/FCDataAnalysis_OverallWorkflow.png", error = FALSE)
```
...
In order to demonstrate CytoPipeline usage, we will use of an example dataset, 
the _HBV mouse dataset_. This dataset was collected during a preclinical study 
aimed at assessing the effect of different therapeutic vaccine regimens 
on the immune response of Hepatitis B Virus transduced mice.

In short, 55 mice, split in 5 different experimental groups, 
according to their vaccine regimen or HBV transduction status, were immunized 
at different timepoints. Upon sacrifice, liver tissue sample fragments 
were collected, treated as to isolate the lymphocytes cell population, 
and subsequently analysed in a flow cytometer for immuno-phenotyping. 
This experiment resulted in the acquisition of 55 `fcs` raw data file, 
with a flow cytometry panel of 12 different channels.

Here we will work on a small subset of this dataset, containing 
two samples, respectively labelled _D91_A01_ and _D93_C01_.
...
which specifies the *PeacoQC* pipeline as described above.
...
```{r step2:plot, out.height=450, out.width=600, fig.height=4.5, fig.width=6, fig.align='center'}
...
Note that the first three bubbles are now highlighted in green, showing that
the first three steps have now produced results. These results have in fact
been saved in a data cache, allowing for further visualization of the results.  

However, as mentioned earlier, the fourth step, 'remove_debris', 
...
correctly executed are not re-run, the results are indeed read from the data 
cache.
...
when looking at the `remove_debris` step, we can notice that this step 
was not totally effective in removing all undesirable events in the FSC-A/SSC-A 
2D representation. 
...
pipeline with the same steps, only we use `nClust = 2` instead of `nClust = 3` 
...
```{r step3:nClust-2, out.height=450, out.width=600, fig.height=4.5, fig.width=6, fig.align='center'}

# 3.

# create pipeline with nClust = 2

expName <- "CytoPipeline_Demo_2C"

jsonFile <- ("CytoPipeline_Demo_pipeline_2C.json")
...
with this new `nClust = 2` parameter : 
...
if (!interactive()) {
    ...
    p3 <- CytoPipelineGUI::plotDiffFlowFrame(
        ...
        experimentNameFrom = expName,
        whichQueueFrom = "pre-processing",
        sampleFileFrom = 1, #D91_G01
        flowFrameNameFrom = "remove_doublets_obj",
        xChannelLabelFrom = "FSC-A : NA",
        yChannelLabelFrom = "SSC-A : NA",
        experimentNameTo = expName,
        whichQueueTo = "pre-processing",
        sampleFileTo = 1, #D91_G01
        flowFrameNameTo = "remove_debris_obj",
        xChannelLabelTo = "FSC-A : NA",
        yChannelLabelTo = "SSC-A : NA",
        useAllCells = TRUE,
...
In this section, we will run the alternative pipeline using *flowAI* as 
QC in time method. As mentioned in the introduction, this implies a slightly 
order of the steps as well. The pipeline is described in the following 
json file:

```{r jsonFileFlowAIShow, comment = ''}
jsonFile <- ("CytoPipeline_Demo_pipeline_flowAI.json")
cat(readLines(jsonFile), sep = '\n')
```
...
pipL_flowAI <- CytoPipeline(jsonFile,
                            experimentName = "CytoPipeline_Demo_flowAI",
                            sampleFiles = mySamples)
